...
const HIDDEN_OPTIMIZE_MODEL_IDS = new Set<PromptOptimizerModelId>(['step-3.5-flash']);
...
const PromptNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const [selectedModel, setSelectedModel] = useState<OptimizeModelId>(nodeData.optimizeModel || 'auto');
  ...
  const handleAiSearch = useCallback(async () => {
    ...
    try {
      ...
      const preferredProvider = selectedModel === 'auto' ? undefined : selectedModel;
      const result = await promptOptimizerService.optimizePrompt(
        ...
        promptType, agentCategory, undefined, preferredProvider
      );
  ...
  const handleOptimize = useCallback(async () => {
    ...
    try {
      ...
      // 使用用户选择的模型（auto 时传 undefined 让后端自动路由）
      const preferredProvider = selectedModel === 'auto' ? undefined : selectedModel;
      const result = await promptOptimizerService.optimizePrompt(
        localPrompt, promptType, agentCategory, undefined, preferredProvider
      );
      if (result.success && result.optimizedPrompt) {
        ...
        toast.success('提示词已增强优化', {
          description: `使用模型: ${selectedModel === 'auto' ? '智能路由' : OPTIMIZE_MODELS.find(m => m.id === selectedModel)?.name}`,
        });
      } else {
                        ...
                        {VISIBLE_OPTIMIZE_MODELS.map((model) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {'displayModel' in model && model.displayModel !== '自动选择高质量文本模型' && (
