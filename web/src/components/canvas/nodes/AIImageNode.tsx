...
const DEFAULT_AI_IMAGE_MODEL_ID = 'doubao-seedream-5-0-pro';
const DEFAULT_AI_IMAGE_MODEL_PROVIDER = 'ai-node-router';
...
const IMAGE_MODEL_PRESETS: AIImageModelPreset[] = [
  ...
  },
  {
    ...
    shortLabel: 'MiniMax Image-01',
    provider: 'minimax',
    badge: '2K',
    modes: ['text_to_image', 'image_to_image', 'reference'],
    fields: ['aspectRatio', 'imageCount', 'promptEnhancer'],
    aspectRatios: ['1:1', '16:9', '9:16', '4:3', '3:4', '3:2', '2:3', '21:9'],
    countOptions: [1, 2, 4],
    referenceKinds: ['character'],
    capabilityTags: ['2K高清', '8种比例', '图生图', '参考图'],
    defaults: {
      aspectRatio: '16:9',
      imageCount: 1,
      promptEnhancer: true,
  ...
  },
  {
    ...
    shortLabel: 'SenseNova U1 Fast',
    provider: 'sensenova',
    badge: '2K',
    modes: ['text_to_image'],
    fields: ['aspectRatio', 'imageSize', 'imageCount'],
    aspectRatios: ['1:1', '2:3', '3:2', '3:4', '4:3', '4:5', '5:4', '16:9', '9:16', '21:9', '9:21'],
    imageSizes: ['2K'],
    countOptions: [1, 2, 4],
    capabilityTags: ['2K高清', '11种比例', '性价比'],
    note: '练习模型，品质较差',
    defaults: {
      aspectRatio: '16:9',
      imageSize: '2K',
  ...
  },
  {
    ...
    label: 'StepFun Image Edit 2',
    shortLabel: 'StepFun Image Edit 2',
    provider: 'stepfun',
    badge: '编辑',
    modes: ['text_to_image', 'image_to_image', 'reference'],
    fields: ['aspectRatio', 'imageSize', 'cfgScale', 'seed'],
    aspectRatios: ['1:1', '16:9', '9:16', '4:3', '3:4'],
    imageSizes: ['1024x1024', '768x1360', '1360x768', '896x1184', '1184x896'],
    countOptions: [1],
    referenceKinds: ['style', 'color', 'character', 'structure', 'remix'],
    capabilityTags: ['文生图', '图生图', '图片编辑'],
    note: '练习模型，品质较差',
    defaults: {
      aspectRatio: '16:9',
      imageSize: '1360x768',
  ...
  },
  {
    ...
    shortLabel: 'Agnes Image 2.1 Flash',
    provider: 'agnes',
    badge: '快速',
    modes: ['text_to_image', 'image_to_image'],
    fields: ['aspectRatio', 'imageSize', 'imageCount', 'seed'],
    aspectRatios: ['1:1', '4:3', '3:4', '16:9', '9:16', '3:2', '2:3', '21:9', '9:21'],
    imageSizes: [
      '1024x1024',
      '1024x768',
      '768x1024',
    ...
    countOptions: [1, 2, 4],
    referenceKinds: ['style', 'color', 'structure', 'remix'],
    capabilityTags: ['快速生成', '低成本', '4K', '图生图', '高信息密度', '构图保持'],
    note: '4K模型，生成较慢',
    defaults: {
      aspectRatio: '16:9',
      imageSize: '2560x1440',
...
function isMergedAiImageChannelPreset(preset: AIImageModelPreset) {
  ...
  if (provider !== 'ai-node-router' && lower.includes('doubao-seedream-5-0-pro')) return true;
  // 豆包 Seedream 5.0 Pro / Pro 同样只保留合并入口；不再按渠道重复展示。
  return (
    provider !== 'ai-node-router' &&
...
function buildDynamicImageModelPreset(model: UnifiedModelConfig): AIImageModelPreset {
  ...
  return {
    ...
    badge: model.isAvailable ? '已连接' : '未配置',
    modes,
    fields,
    aspectRatios,
    imageSizes,
...
export function mergeImageModelPresets(
  ...
  return [
    ...COMBINED_AI_IMAGE_MODEL_PRESETS.map((preset) => ({
      ...preset,
      note:
        preset.id === 'doubao-seedream-5-0-pro'
          ? '仅小天6，故障时不会自动降级为 2K'
          : '后台自动跳过失效、限流或未配置的渠道',
    })),
    ...otherPresets,
  ...
  return (
    ...
    >
      <div className="max-h-[260px] overflow-y-auto pr-1 custom-scrollbar">
        {groupImageModelsByProvider(modelPresets).map((group) => (
