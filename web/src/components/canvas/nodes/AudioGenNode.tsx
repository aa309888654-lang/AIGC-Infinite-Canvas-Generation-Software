...
const AudioGenNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!membershipStore.membership?.isLoggedIn) {
    ...
    try {
      ...
      if (
        getProviderForAudioModel(currentModelId) === 'stepfun' &&
        currentModelId === 'stepaudio-2.5-tts'
      ) {
        (params as AudioGenerationParams & { instruction?: string }).instruction = content.slice(
          0,
          200
        );
      }
  ...
  const handlePreviewVoice = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        (params as AudioGenerationParams & { instruction?: string }).instruction =
          '自然清晰，语气友好，适合试听音色';
      }
