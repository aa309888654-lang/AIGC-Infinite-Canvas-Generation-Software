...
type AIVideoModelPreset = OfficialVideoModelCapability & { isCustomModel?: boolean };
...
const VIDEO_MODEL_PRESETS: AIVideoModelPreset[] = getVideoControllerModelPresets();
const DEFAULT_VIDEO_MODEL =
  VIDEO_MODEL_PRESETS.find((item) => item.id === 'agnes-video-v2.0') || VIDEO_MODEL_PRESETS[0];
...
function groupVideoModelsByProvider(
  ...
  const order = ['自定义模型', '小天4', '体验版模型组', '豆包', '其他模型'];
...
function getVideoModelShortLabel(model: AIVideoModelPreset): string {
  ...
  if (model.id === 'agnes-video-v2.0') return 'Agnes Video V2.0';
  const baseLabel =
...
function AIVideoNode({ data, id, selected }: NodeProps) {
  ...
  const requestedModelId = String(
    nodeData.modelId || nodeData.params?.modelId || 'agnes-video-v2.0'
  );
  ...
  const model =
    ...
    getVisibleVideoModel('agnes-video-v2.0');
  ...
  const handleExecuteVideo = useCallback(
    ...
    [id, inputState, isProcessing, localPrompt, mode, model, params, acquireLock, releaseLock]
  );
                          ...
                          <div key={group.label} className="space-y-1.5">
                            ...
                            {group.options.map((item) => (
                              ...
                              >
                                ...
                                {model.id === item.id ? (
                                  <span className="h-2 w-2 shrink-0 rounded-full bg-cyan-200 shadow-[0_0_10px_rgba(103,232,249,0.75)]" />
                                ) : null}
                              </button>
                            ))}
                          </div>
