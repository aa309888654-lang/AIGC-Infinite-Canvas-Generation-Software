...
const SettingsPanel = ({ isOpen, onClose, initialTab }: SettingsPanelProps) => {
  const [activeTab, setActiveTab] = useState<SettingsTab>('interface');
  const [showSponsorModal, setShowSponsorModal] = useState(false);
  const [sponsorData, setSponsorData] = useState<{qrcodeUrl: string; description: string}>({
    qrcodeUrl: PUBLIC_URLS.sponsorQrcode,
    description: '请扫描上方二维码进行赞助支持'
  });
  const [appIconUrl, setAppIconUrl] = useState<string>(PUBLIC_URLS.appIcon);
  ...
  const loadSponsorInfo = async () => {
    try {
      const resp = await axios.get(`${API_BASE_URL}/public/sponsor`);
      if (resp.data?.success && resp.data?.data) {
        setSponsorData(resp.data.data);
      }
    } catch (err) {
  ...
  const loadAppIcon = async () => {
    try {
      const resp = await axios.get(`${API_BASE_URL}/public/sponsor/icon`);
      if (resp.data?.success && resp.data?.data) {
        const baseUrl = resp.data.data.iconUrl;
        const separator = baseUrl.includes('?') ? '&' : '?';
        setAppIconUrl(`${baseUrl}${separator}t=${Date.now()}`);
      }
    } catch (err) {
      console.error('加载应用图标失败:', err);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 p-3 overflow-y-auto flex items-center justify-center">
                    ...
                    {isLoggedIn ? (
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                        <div className="flex items-center gap-4">
                          <div className="w-16 h-16 rounded-full bg-gradient-to-r from-[#4B21FF] to-[#AF52DE] flex items-center justify-center overflow-hidden">
                            {membership?.avatar ? (
                              <img src={membership.avatar || ''} alt="avatar" className="w-full h-full object-cover" />
                            ) : (
                              <User className="w-8 h-8 text-white" />
                            )}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="text-white font-medium">{membership?.username || '用户'}</div>
                            <div
                              className="max-w-full overflow-hidden break-all text-sm leading-5 text-white/50 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]"
                              title={membership?.email || '未设置邮箱'}
                            >
                              {membership?.email || '未设置邮箱'}
                            </div>
                            <div className="flex items-center gap-1 mt-1" style={{ display: 'none' }}>
                              <Crown className="w-3 h-3 text-yellow-400" />
                              <span className="text-xs text-yellow-400">
                                {getMembershipLevelIcon(membership?.membershipLevel)}
                                {getMembershipLevelName(membership?.membershipLevel, membership?.membershipDisplayName)}
                              </span>
                            </div>
                          </div>
                        </div>
                        <button 
                          className="mt-4 w-full py-2 rounded-lg bg-gradient-to-r from-[#4B21FF] to-[#AF52DE] text-white text-sm font-medium hover:opacity-90 transition-opacity"
                          onClick={() => navigateTo('/')}
                          style={{ display: 'none' }}
                        >
                          查看会员中心
                      ...
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                        <div className="flex items-center gap-4">
                          <div className="w-16 h-16 rounded-full bg-gradient-to-r from-[#4B21FF] to-[#AF52DE] flex items-center justify-center">
                            <User className="w-8 h-8 text-white" />
                          </div>
                          <div>
                            <div className="text-white font-medium">访客用户</div>
                            <div className="text-sm text-white/50">未登录</div>
                          </div>
                        </div>
                        <button 
                          className="mt-4 w-full py-2 rounded-lg bg-gradient-to-r from-[#4B21FF] to-[#AF52DE] text-white text-sm font-medium hover:opacity-90 transition-opacity"
              ...
              {activeTab === 'about' && (
                <div className="space-y-6">
                  <div className="text-center py-8">
                    ...
                    <h2 className="text-xl font-bold text-white">AICG-创意工作台</h2>
                  ...
                  <div className="space-y-4 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between">
                      <div>
                        <div className="text-sm text-white/80">检查更新</div>
                        <div className="text-xs text-white/50">{updateStatus || '点击按钮检查最新版本'}</div>
                      </div>
                      <button 
                        onClick={checkForUpdates}
                        disabled={isCheckingUpdate}
                        className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white/70 text-sm transition-all flex items-center gap-2 disabled:opacity-50"
                      >
                        {isCheckingUpdate ? (
                          <>
      ...
      {showSponsorModal && (
        ...
        >
          ...
          <div 
            className="relative bg-[#1a1a1a] rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl"
          ...
          >
            ...
            <h3 className="text-xl font-bold text-white text-center mb-4">❤️ 感谢您的支持</h3>
            
            <div className="space-y-4">
              <p className="text-sm text-white/70 text-center">
                {sponsorData.description || '您的支持是我持续开发的最大动力！扫码请您备注邮箱，将为您开通专属权限。'}
              </p>
              
              <div className="bg-white rounded-xl p-4">
                <img 
                  src={sponsorData.qrcodeUrl}
                  alt="赞助二维码"
