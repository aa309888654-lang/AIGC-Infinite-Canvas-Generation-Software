...
export const useUnifiedAPIConfigStore = create<APIConfigState>()((set, get) => ({
  ...
  fetchUserCredentialConfigs: async () => {
    if (!getAuthToken()) {
      set({ userCredentialConfigs: {}, userCredentialStorage: { usedBytes: 0, limitBytes: 2 * 1024 * 1024 } });
      return;
    }
    try {
  ...
  fetchProviderConfigs: async (options) => {
    ...
    providerConfigsFetchPromise = (async () => {
      try {
        ...
        if (hasAuth) {
          // 已登录：从私有 API 获取完整 provider 配置
          const response = await aiProviderService.getProviders();
          ...
          for (const p of providers) {
            configs[p.provider] = {
              ...
              baseUrl: p.endpoint || '',
              authType: p.config?.authType || 'api-key',
              models: (p.models || []).map(m => {
                ...
                return {
                  ...
                  type: obj.type,
                  providerModel: obj.providerModel,
                  capabilities: obj.capabilities,
                  modelCategory: obj.modelCategory,
                  requiredInputs: obj.requiredInputs,
                  routeProvider: obj.routeProvider,
                  configuredProvider: obj.configuredProvider,
                  disabledReason: obj.disabledReason,
                  fallbackModelId: obj.fallbackModelId,
                  fallbackProvider: obj.fallbackProvider,
                  keyScope: obj.keyScope,
                  isActive: obj.isActive,
                  maxResolution: obj.maxResolution,
                  maxDuration: obj.maxDuration,
                  supportedResolutions: obj.supportedResolutions,
                  supportedAspectRatios: obj.supportedAspectRatios,
                  supportedModes: obj.supportedModes,
                  defaultParams: obj.defaultParams,
                };
              }),
              features: p.config?.features as Record<string, boolean> | undefined,
              officialDocsUrl: p.config?.officialDocsUrl,
              isCustomModel: p.config?.isCustomModel === true,
              mediaType: p.config?.mediaType === 'video' ? 'video' : p.config?.mediaType === 'image' ? 'image' : undefined,
            };
          }
        }

        // 未登录或已登录：都从公共 API 获取活跃 provider 信息作为补充
        try {
        ...
        if (hasAuth) await get().fetchUserCredentialConfigs();
  ...
  setConfig: (provider: APIProvider, config: Partial<APIAuthConfig>) => {
    ...
    if (getAuthToken() && Object.values(credentialInput).some((value) => typeof value === 'string' && value.length > 0)) {
