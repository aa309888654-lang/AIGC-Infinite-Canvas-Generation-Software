...
const WorkflowMarketplacePanel = lazyWithChunkRecovery(
  () => import('./components/ui/WorkflowMarketplacePanel')
);
...
const ModelMarketplace = lazyWithChunkRecovery(() => import('./components/ui/ModelMarketplace'));
...
function AppContent({
  ...
  const isWorkflowMarketplaceOpen = useAppPanelStore((s) => s.isWorkflowMarketplaceOpen);
  ...
  const isModelMarketplaceOpen = useAppPanelStore((s) => s.isModelMarketplaceOpen);
  ...
  const setIsWorkflowMarketplaceOpen = useAppPanelStore((s) => s.setWorkflowMarketplaceOpen);
  ...
  const setIsModelMarketplaceOpen = useAppPanelStore((s) => s.setModelMarketplaceOpen);
  ...
  const handleImportMarketplaceTemplate = useCallback(
    (templateNodes: Node[], templateEdges: Edge[]) => {
      if (templateNodes.length === 0) {
        addDragDropToast('warning', '这个模板暂时没有可导入的节点');
        return;
      }
      ...
      const offsetX = center.x - minX - 180;
      const offsetY = center.y - minY - 120;

      const importedNodes = templateNodes.map((node) => {
      ...
      store.setNodes([...store.nodes, ...importedNodes]);
      store.setEdges([...store.edges, ...importedEdges]);
      store.setSelectedNodeIds(importedNodes.map((node) => node.id));
      store.setSelectedNodeId(importedNodes[0]?.id ?? null);
      importedNodes.forEach((node) => {
        useVirtualRenderStore.getState().addRecentlyAddedNode(node.id);
      });

      onViewChange('workflow');
      setIsWorkflowMarketplaceOpen(false);
      setTimeout(() => {
        fitViewRef.current({ padding: 0.45, duration: 500, nodes: importedNodes });
      }, 120);
      addDragDropToast('success', `已导入 ${importedNodes.length} 个节点到画布`);
    },
    [addDragDropToast, getCanvasViewportCenter, onViewChange, setIsWorkflowMarketplaceOpen]
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'K') {
        e.preventDefault();
        h.layoutTogglePanel('modelMarketplace');
      }
      ...
      if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'M') {
        e.preventDefault();
        h.layoutTogglePanel('workflowMarketplace');
      }
    ...
    <>
      <Suspense fallback={<LoadingSpinner />}>
        <TopToolbar
          ...
          onOpenWorkflowMarketplace={() => setIsWorkflowMarketplaceOpen(true)}
          onOpenWorkflowDebugger={() => setIsWorkflowDebuggerOpen(true)}
          onOpenModelMarketplace={() => setIsModelMarketplaceOpen(true)}
      ...
      {isWorkflowMarketplaceOpen && (
        <Suspense fallback={<LoadingSpinner />}>
          <WorkflowMarketplacePanel
            isOpen={isWorkflowMarketplaceOpen}
            onClose={() => setIsWorkflowMarketplaceOpen(false)}
      ...
      {isModelMarketplaceOpen && (
        <Suspense fallback={<LoadingSpinner />}>
          <ModelMarketplace
            isOpen={isModelMarketplaceOpen}
            onClose={() => setIsModelMarketplaceOpen(false)}
          />
        </Suspense>
      )}
      {isAPIMonitorOpen && (
