      ...
      {collapsed ? (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-xs font-semibold text-white">{t('task_queue.title')}</p>
              <p className="text-[10px] tabular-nums text-gray-400">
                {activeTask
                  ? `${t('task_queue.generating')} ${activeProgress}%`
                  : stats.total > 0
                    ? t('task_queue.task_count', { count: stats.total })
                    : t('task_queue.no_tasks')}
              </p>
              {activeTask && (
        ...
        <div className="flex flex-col" style={{ height: `${currentHeight}px` }}>
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-[#252528] border-b border-[#2A2A2E] flex-shrink-0">
            <div className="flex items-center gap-3">
              <div>
                <h2 className="text-xs font-semibold text-white">{t('task_queue.title')}</h2>
                <p className="text-[10px] text-gray-400">
                  {stats.processing > 0 
                    ? t('task_queue.processing_count', { count: stats.processing })
                    : stats.total > 0 
                      ? t('task_queue.task_count', { count: stats.total })
                      : t('task_queue.no_tasks')
                  }
