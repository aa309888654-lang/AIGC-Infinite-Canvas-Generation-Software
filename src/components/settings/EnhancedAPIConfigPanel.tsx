...
import { aiProviderService, type CustomModelDiagnostic } from '@/services/ai-provider-service';
...
interface EnhancedAPIConfigPanelProps {
  targetProvider?: string;
  targetModel?: string;
}

export const EnhancedAPIConfigPanel: React.FC<EnhancedAPIConfigPanelProps> = ({ targetProvider, targetModel }) => {
  const {
    configs,
    providerConfigs,
    isLoadingConfigs,
  ...
  const availableOfficialPresets = useMemo(
    () => getDomesticOfficialModelPresets(newModel.mediaType === 'video' ? 'video' : 'image'),
    [newModel.mediaType]
  );
  const officialModelOptions = useMemo(() => {
    if (!selectedOfficialPreset) return [];
    const options = new Map<string, string>();
    for (const option of selectedOfficialPreset.modelOptions || []) {
      options.set(option.id, option.label);
    }
    options.set(selectedOfficialPreset.model, `${selectedOfficialPreset.displayName}（默认）`);
    for (const model of modelRegistry.getModelsByType(newModel.mediaType)) {
      const provider = String(model.provider || '').toLowerCase();
      const routeProvider = String(model.routeProvider || '').toLowerCase();
      if (provider !== selectedOfficialPreset.provider.toLowerCase() && routeProvider !== selectedOfficialPreset.provider.toLowerCase()) continue;
      const id = String(model.providerModel || model.modelId || model.id || '').trim();
      if (id) options.set(id, model.name || id);
    }
    return Array.from(options, ([id, label]) => ({ id, label }));
  }, [newModel.mediaType, selectedOfficialPreset, refreshingModelCatalog]);
  ...
  const filteredProviders = useMemo(() => {
    return Object.entries(providerConfigs).filter(([providerId, config]) => {
      // 默认只展示用户可配置的自定义通道；从模型菜单跳转时保留目标官方通道。
      if (config.isCustomModel !== true && providerId !== targetProvider) return false;
      if (selectedCategory === 'all') return true;
      if (selectedCategory === 'image') return config.mediaType === 'image';
      if (selectedCategory === 'video') return config.mediaType === 'video';
      return true;
    });
  }, [selectedCategory, providerConfigs, targetProvider]);
  ...
  const handleRefreshModelCatalog = async () => {
    ...
    try {
      await Promise.all([
        fetchProviderConfigs({ force: true }),
        modelRegistry.refreshFromBackend(),
      ]);
      toast.success('模型列表已联网更新');
    } catch (error) {
      logger.warn('[EnhancedAPIConfig] 模型列表更新失败', error);
      toast.error('模型列表更新失败，请检查网络后重试');
    } finally {
    ...
    <div className="h-full flex flex-col bg-gray-900 text-white">
      <div className="p-6 border-b border-gray-700">
        ...
        <div className="mt-4 rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4">
            <div className="mb-3 flex items-center justify-between gap-2 text-sm font-medium text-white">
              <span className="flex items-center gap-2"><Plus className="h-4 w-4 text-cyan-300" />添加图片或视频模型 API</span>
              <button type="button" onClick={handleRefreshModelCatalog} disabled={refreshingModelCatalog} className="flex items-center gap-1 rounded-md border border-cyan-500/30 px-2 py-1 text-xs text-cyan-300 transition hover:bg-cyan-500/10 disabled:cursor-wait disabled:opacity-60">
                <RefreshCw className={cn('h-3.5 w-3.5', refreshingModelCatalog && 'animate-spin')} />联网更新模型
              </button>
            </div>
            <p className="mb-3 text-xs text-gray-400">仅接入图片与视频生成模型。自定义接口需兼容 OpenAI 图片或视频生成协议；API Key 仅加密保存在本机空间。</p>
            <div className="grid gap-3 md:grid-cols-2">
              <select value={newModel.mediaType} onChange={(e) => setNewModel(createEmptyCustomModel(e.target.value as CustomMediaType))} className="rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white"><option value="image">AI 图片模型</option><option value="video">AI 视频模型</option></select>
              <select value={newModel.presetId} onChange={(e) => { const value = e.target.value; const applied = applyDomesticOfficialModelPreset(value); setNewModel(applied ? { ...newModel, ...applied } : { ...createEmptyCustomModel(newModel.mediaType), connectionMode: 'proxy' }); }} className="rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white"><option value="">选择模型</option>{availableOfficialPresets.map((preset) => <option key={preset.id} value={preset.id}>{preset.displayName}</option>)}<option value="custom">自定义模型（最后一项）</option></select>
              {newModel.connectionMode === 'official' ? <input value={newModel.displayName} readOnly aria-label="官方模型名称" className="rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-gray-400" /> : <input value={newModel.displayName} onChange={(e) => setNewModel({ ...newModel, displayName: e.target.value })} placeholder="自定义模型名称" className="rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white" />}
              <div className="min-w-0">
                {newModel.connectionMode === 'official' ? (
                  <select value={newModel.upstreamModel} onChange={(e) => setNewModel({ ...newModel, upstreamModel: e.target.value })} disabled={!selectedOfficialPreset} aria-label="选择官方模型 ID" className="w-full rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-60">
                    <option value="">{selectedOfficialPreset ? '选择内置热门模型' : '请先选择模型'}</option>
                    {officialModelOptions.map((option) => <option key={option.id} value={option.id}>{option.label} · {option.id}</option>)}
                  </select>
                ) : (
                  <input value={newModel.upstreamModel} onChange={(e) => setNewModel({ ...newModel, upstreamModel: e.target.value })} placeholder="上游模型 ID" className="w-full rounded-lg border border-gray-600 bg-gray-800 px-3 py-2 text-sm text-white" />
                )}
              </div>
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-gray-400">
                        {targetProvider === providerId && targetModel
                          ? `当前模型：${targetModel}`
