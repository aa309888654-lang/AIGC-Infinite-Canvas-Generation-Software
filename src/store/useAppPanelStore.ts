...
export type SettingsTab = 'interface' | 'performance' | 'api-config' | 'llm-config' | 'shortcuts' | 'paths' | 'storage';
...
interface AppPanelState {
  ...
  settingsTarget?: { provider?: string; model?: string };
  ...
  setSettingsOpen: (open: boolean, tab?: SettingsTab, target?: { provider?: string; model?: string }) => void;
...
export const useAppPanelStore = create<AppPanelState>()((set) => ({
  ...
  setSettingsOpen: (open, tab, target) => set((s) => ({
    isSettingsOpen: open,
    settingsActiveTab: tab ?? s.settingsActiveTab,
    settingsTarget: target ?? (open ? s.settingsTarget : undefined),
  })),
