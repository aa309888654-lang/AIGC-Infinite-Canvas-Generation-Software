...
function normalizeStoryboardImageParams(
  ...
  imageParams.modelId = 'doubao-seedream-5-0-lite';
  imageParams.modelProvider = 'doubao' as any;
  (imageParams as any).model = 'doubao-seedream-5-0-lite';
  (imageParams as any).provider = 'doubao';
}
...
function inferVideoProvider(modelId?: unknown, provider?: unknown): string {
  ...
  const normalizedModel = String(modelId || '')
    .trim()
    .toLowerCase();
  if (!normalizedModel) return 'doubao';
...
function sanitizeVideoGenerationParams<T extends Record<string, any>>(params: T): T {
  ...
  // 保留原始模型，让后端明确返回模型不可用，而不是静默切换到其他模型。
  return params;
}
...
export async function executeSingleNode(nodeId: string): Promise<void> {
  ...
  // 登录与会员系统已移除：去掉登录态前置拦截，仅保留配额/权限探测
  if (feature) {
    const permOptions: {
      provider?: string;
      model?: string;
  ...
  if (isVideo) {
    const provider = (params.modelProvider ||
      ...
      'doubao') as string;
    const modelId = (params.modelId as string) || 'doubao-seedance-2-0';
  ...
  if (isMultiAngleNode) {
    ...
    const modelId = String(params.modelId || params.model || 'doubao-seedream-5-0-lite');
  ...
  try {
    ...
    if (isVideo) {
      const resolvedVideoModelId = String(
        paramsView.modelId || paramsView.model || 'doubao-seedance-2-0'
      );
  ...
  async executeVideoGen({ id, params }: { id: string; params: VideoGenerationParams }) {
    ...
    try {
      ...
      if (!cleanParams.modelId && !(cleanParams as any).model) {
        (cleanParams as any).modelId = 'doubao-seedance-2-0';
      }
      const requestedVideoCount = normalizeRequestedVideoCount(
